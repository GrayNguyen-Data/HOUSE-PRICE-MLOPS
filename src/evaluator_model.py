...
class EvaluatorModelStratery(ABC):

    def evaluator(self, model: RegressorMixin, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
        pass
class RegressionEvaluatorModel(EvaluatorModelStratery):
    def evaluator(self, model: RegressorMixin, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
...
class EvaluatorModel:
    def __init__(self, strategy: EvaluatorModelStratery):
        self._strategy = strategy

    def set_strategy(self, strategy: EvaluatorModelStratery):
    ...
    def evaluate(self, model: RegressorMixin, X_test: pd.DataFrame, y_test: pd.Series) -> dict: 
        ...
        self._strategy.evaluator(model, X_test, y_test)
