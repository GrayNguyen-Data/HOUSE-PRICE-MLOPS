...
logging.basicConfig(level=logging.INFO, format="%(asictime)s - %(levelname)s - %(message)s")
...
class ZScoreOutlierDetection(OutlierDetectionStrategy):
    ...
    def detected_outlier(self, df):
        ...
        logging.info("Hoàn tất việc tìm kiếm outlier với threshold={self.threshold}")
        return outlier
...
class OutlierDetector:
    ...
    def detected_outlier(self, df: pd.DataFrame) -> pd.DataFrame:
        logging.info("Thực thi phương pháp xử lý oulier đã chọn")
        return self._strategy.detected_outliier(df)
    
    def handle_outlier(self, df: pd.DataFrame, method="remove", **kwargs) -> pd.DataFrame:
        outliers = self.detected_outlier(df)
        if method == 'remove':
            logging.info("Xóa các oulier của dataset")
            df_clean = df[(~outliers).all(axis = 1)]
        elif method == 'cap':
            logging.info("Giowis hạn outlier trong dataset")
            """
            ...
            logging.info("Không áp úng method nào và không có outkier nào được xử lý.")
            return df
    ...
    def visualize_outlier(self, df: pd.DataFrame, features: list):
        ...
        logging.info("Đã vẽ xông các biểu đồ boxplot cho các feature.")
